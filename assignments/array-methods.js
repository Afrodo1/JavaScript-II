...
//const fullNames = runners.map((name) => {
...
const firstNamesAllCaps = runners.map((name) =>{
...
runnersLargeSizeShirt = runners.filter((name)=> {  
...
const donate = runners.length;

ticketPriceTotal = runners.reduce((acc, donateAmt) => {

  return acc += donateAmt.donation;
